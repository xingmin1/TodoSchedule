// Code generated by SynkAdapter plugin. Do not edit this file.
package com.example.todoschedule.`data`.sync.adapter

import com.example.todoschedule.`data`.database.entity.CourseEntity
import com.tap.synk.adapter.SynkAdapter
import com.tap.synk.encode.MapEncoder
import com.tap.synk.resolver.IDResolver
import javax.inject.Singleton

@Singleton
public class CourseEntitySynkAdapter(
    private val idResolver: IDResolver<CourseEntity> = CourseEntityIDResolver(),
    private val mapEncoder: MapEncoder<CourseEntity> = CourseEntityMapEncoder(),
) : SynkAdapter<CourseEntity>, IDResolver<CourseEntity> by idResolver, MapEncoder<CourseEntity> by
        mapEncoder
