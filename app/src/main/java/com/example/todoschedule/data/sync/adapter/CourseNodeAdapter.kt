...
class CourseNodeAdapter @Inject constructor() :
    ...
    /* Synk-Adapter 接口映射 */
    override fun resolveId(crdt: CourseNodeEntity): String = key(crdt)

    override fun encode(crdt: CourseNodeEntity): Map<String, String> =
        serialize(crdt).mapValues { it.value?.toString() ?: "" }

    override fun decode(map: Map<String, String>): CourseNodeEntity =
